Game.registerMod("Auto CCS",{
    init:function(){
        ...
        //MOD.Garden = Game.Objects.Farm.minigame
        //MOD.seeds = MOD.Garden.plantsById
    ...
    buildingPicker:function(){
        ...
        if (Game.cookies > this.getPrice(this.Bestbuilding) && this.buildingsScore >= this.UpgradeScore){
            currBuyMode = Game.buyMode;
            if (Game.buyMode == -1) {
                Game.buyMode = 1
            }
    ...
    upgradepicker:function(){
        ...
        if (this.BestUpgrades != undefined) {
            if (Game.cookies >= this.getUpPrice(this.BestUpgrades) && this.UpgradeScore >= this.buildingsScore){
                if (this.upgradetypes[this.BestUpgrades] == "tf") {
                    Game.UpgradesById[this.BestUpgrades].buy();
                    this.tfboost = this.getTfBoost(this.BestUpgrades, this.tfboost);
                }
                else{
                    Game.UpgradesById[this.BestUpgrades].buy();
                }
    ...
    getUpPrice:function(id){
        return Game.UpgradesById[id].basePrice;
    },
    ...
    getTfBoost:function(id, currentBoost){
        if(id == 3){
            return 0.1;
        }
        else if(id == 4){
            return currentBoost* 5;
        }
        else if(id == 5){
            return currentBoost* 10;
        }
        else{
            return currentBoost* 20;
        }
    },
    x2upgrades:function(id){
        price = Game.UpgradesById[id].getPrice();
        if (id in [0,1,2]) {
            totalCps = Game.Objects.Cursor.storedTotalCps;
            return(((this.clickCps() + totalCps) * 2)/price); 
        }
    ...
    GrandmaTypeMath:function(id){
        incremetnal = Game.UpgradesById[id].buildingTie.id - 1; //sneaky work around for not having to hard code
    ...
    calculateGains:function(listitem,id){
        switch (listitem) {
            ...
            default: // any errors in list as well as ig upgrades will just retrun 0 and nothing will happen
                return 0;
        }
    },
